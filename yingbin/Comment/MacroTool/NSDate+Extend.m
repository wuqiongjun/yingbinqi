...
+ (NSString *)timeStringFromTimestamp:(NSInteger)timestamp formatter:(NSString *)format {
    ...
    NSTimeZone *timeZone = [NSTimeZone timeZoneWithName:@"Asia/Beijing"];
    [formatter setTimeZone:timeZone];
    NSString *mytime = [NSString stringWithFormat:@"%ld",timestamp];
    NSDate *timeDate;
    if (mytime.length >10) {
        timeDate = [self dateFromTimestamp:timestamp / 1000];
    }else {
        timeDate = [self dateFromTimestamp:timestamp];
    }
...
+ (NSInteger)getSumOfDaysMonth:(NSInteger)month inYear:(NSInteger)year {
    ...
    NSString *dateString = [NSString stringWithFormat:@"%ld-%ld", (long)year, (long)month];

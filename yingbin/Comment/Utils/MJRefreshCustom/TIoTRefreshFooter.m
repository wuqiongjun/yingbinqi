...
- (void)prepare{
    ...
    [self setTitle:LOCSTR(@"加载中...")  forState:MJRefreshStateRefreshing];
    [self setTitle:LOCSTR(@"没有更多了!") forState:MJRefreshStateNoMoreData];
}
...
- (UILabel *)stateLabel
{
    if (!_stateLabel) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:_stateLabel action:@selector(loadMoreData)];
